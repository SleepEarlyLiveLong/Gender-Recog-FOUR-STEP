...
%         VOICES(i).num = min(length(file_list{i})-2,50+round(5*rand()));   % 随机抽取部分数据
    ...
    if strcmp(control.classifier,'DistCompare')==1
        errordlg('错误！这个分类器的实现作为拓展内容，需要用户自己编写','错误提示');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
hs = msgbox({'软件版本说明:';'';'Version: 0.1 ';'';...
             'Author: Chen Tianyang';'';...
             'Data:2019-03-15';''},'Version Information');
%改变字体大小
ht = findobj(hs, 'Type', 'text');
set(ht,'FontSize',8);
%改变对话框大小
